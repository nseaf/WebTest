...
| **浏览器自动化** | browser-use CLI + Skill | 主要工具，支持多Chrome实例 |
...
- `mcp__playwright__browser_snapshot` - 获取可访问性树
...
**重要**: Playwright MCP 必须通过 `--proxy-server` 命令行参数配置代理。
...
**前置条件**:
- 在 Burp Suite 中安装 CA 证书到系统信任列表
- 建议关闭 Burp 的 "Intercept" 模式
- 修改 `.mcp.json` 后需要重启 MCP 服务
