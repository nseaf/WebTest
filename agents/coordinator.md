...
2. 更新 result/sessions.json
   ...
   ↓
3. 同步到 BurpBridge
   主要方式：browser-use cookies get --json
   转换格式后调用 mcp__burpbridge__configure_authentication_context
   ↓
4. 记录同步时间
...
**获取 Cookie（browser-use CLI）**

```bash
# 使用 browser-use CLI 获取 Cookie（主要方式）
browser-use cookies get --json
# 输出: [{"name": "session", "value": "abc123", "domain": "example.com"}]
```

**配置角色认证上下文**

```javascript
// browser-use 输出格式转换为 BurpBridge 格式
...
// 或直接导入 browser-use/Playwright 格式的 Cookie
