---
name: agent-contract
description: "Agent合约模板，定义输出格式、截断防御、门控条件。Coordinator调用子Agent前注入，确保Agent行为可控、输出可追踪。"
---
...
## 与opencode-agents的对应关系

此Skill借鉴了opencode-agents的agent-contract Skill：

| opencode-agents | WebTest |
|-----------------|---------|
| HEADER段 | HEADER段（包含STATE/COVERAGE） |
| TRANSFER BLOCK | TRANSFER BLOCK（包含发现记录） |
| AGENT_OUTPUT_END | AGENT_OUTPUT_END（截断哨兵） |
| 搜索路径/排除目录 | Session参数/CDP URL |
| 工具调用预算 | Token预算 |
| 三问法则 | 三问法则（轮次判定） |