...
```bash
browser-use --session admin_001 cookies get
```

Windows 建议：

```powershell
powershell -ExecutionPolicy Bypass -File scripts/browser-use-utf8.ps1 --session admin_001 cookies get
```
...
- Cookie 值必须来自 `browser-use --session ... cookies get` 的真实输出。
